...
#equivalent curl command, too complex for now 
#curl -i -XPOST 'http://localhost:99/write?db=SCAinfluxdb' --data-binary 'liftwork,LiftWorkDescription=workdescriptiontext,LiftWorkId=workidtext,LiftWorkStartTime=liftworkstarttimetext,LiftWorkStopTime=liftworkstoptimetext,LiftWorkLocationId=liftworklocationtext,LiftLoadCenter=0.00,LiftWorkWeight=0.00'

# simpler POST, using this for initial demo
